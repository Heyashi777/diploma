...
for per in periods:
    ...
    tarin_data = tarin_data[(tarin_data["beta"] >= 0.2) & (tarin_data["returns"] >= 0)]
    moex = data_moex[(data_moex.index >= per) & (data_moex.index < stop)].copy()
    if i >= 1:
        ...
        sub_result = Portfel(
            ...
            expected_return * 2 * 100,  # noqa: F821
            income["income"].mean() * 100 * 2,
            moex["MOEX"].mean() * 2 * 100,
        )  # noqa: F821
    ...
    expected_return = (tarin_data["returns"] * tarin_data["weight"]).sum()
